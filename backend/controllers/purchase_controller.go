...
func GetPurchases(c *fiber.Ctx) error {
	...
	// Ambil hanya data purchase, tanpa preload Product
	if err := db.Select("purchase_id, purchase_code, product_id, quantity, cost_price, total_cost, date").Find(&purchases).Error; err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": err.Error()})
	}
...
func GetPurchase(c *fiber.Ctx) error {
	...
	id := c.Params("id")
	// Ambil data purchase, tanpa preload Product
	if err := db.Select("purchase_id, purchase_code, product_id, quantity, cost_price, total_cost, date").First(&purchase, "purchase_id = ?", id).Error; err != nil {
